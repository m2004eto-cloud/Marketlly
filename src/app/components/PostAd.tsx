...
const colorOpts = [
  ...
  { name: "Tan", hex: "#d2b48c" }, { name: "Teal", hex: "#0d9488" }, { name: "White", hex: "#ffffff" }, { name: "Pearl White", hex: "#f8f4f0" },
...
// cats is now built inside the component using catalog context
...
// Field definitions are built dynamically inside the component using catalog context.
// The static arrays below are kept only as type references — actual opts come from CatalogContext.
const classifiedsFields: Field[] = [
  { key: "condition", label: "Condition", type: "pill-single", opts: [] },
  { key: "brand", label: "Brand", type: "text" },
  { key: "warranty", label: "Warranty", type: "pill-single", opts: ["Yes - Manufacturer", "Yes - Seller", "No"] },
];
...
export function PostAd({ onBack, onCreated, onAuctionCreated }: Props) {
  ...
  const motorsFields: Field[] = [
    ...
    { key: "regionalSpecs", label: "Regional Specs", type: "pill-single", opts: catalog.motorsFields.regionalSpecs },
    { key: "sellerType", label: "Seller Type", type: "pill-single", opts: catalog.motorsFields.sellerType },
    { key: "bodyType", label: "Body Type", type: "pill-single", opts: catalog.motorsFields.bodyType },
    { key: "seats", label: "Seats", type: "pill-single", opts: catalog.motorsFields.seats },
    { key: "transmission", label: "Transmission Type", type: "pill-single", opts: catalog.motorsFields.transmission },
    { key: "fuel", label: "Fuel Type", type: "pill-single", opts: catalog.motorsFields.fuel },
    { key: "badges", label: "Badges", type: "pill-multi", opts: catalog.motorsFields.badges },
    { key: "exportStatus", label: "Export Status", type: "pill-single", opts: catalog.motorsFields.exportStatus },
    { key: "exteriorColor", label: "Exterior Color", type: "colors", opts: catalog.colorOpts },
    { key: "interiorColor", label: "Interior Color", type: "colors", opts: catalog.colorOpts },
    { key: "horsepower", label: "Horsepower", type: "pill-single", opts: catalog.motorsFields.horsepower },
    { key: "engineCapacity", label: "Engine Capacity (Cc)", type: "pill-single", opts: catalog.motorsFields.engineCapacity },
    { key: "doors", label: "Doors", type: "pill-single", opts: catalog.motorsFields.doors },
    { key: "warranty", label: "Warranty", type: "pill-single", opts: catalog.motorsFields.warranty },
    { key: "cylinders", label: "Number Of Cylinders", type: "pill-single", opts: catalog.motorsFields.cylinders },
    { key: "driverAssistance", label: "Driver Assistance & Safety", type: "pill-multi", opts: catalog.motorsFields.driverAssistance },
    { key: "comfort", label: "Comfort & Convenience", type: "pill-multi", opts: catalog.motorsFields.comfort },
    { key: "entertainment", label: "Entertainment & Technology", type: "pill-multi", opts: catalog.motorsFields.entertainment },
    { key: "exteriorFeatures", label: "Exterior Features", type: "pill-multi", opts: catalog.motorsFields.exteriorFeatures },
  ];

  const dynamicClassifiedsFields: Field[] = [
    { key: "condition", label: "Condition", type: "pill-single", opts: catalog.motorsFields.condition },
    { key: "brand", label: "Brand", type: "text" },
    { key: "warranty", label: "Warranty", type: "pill-single", opts: ["Yes - Manufacturer", "Yes - Seller", "No"] },
  ];
  ...
  const fieldsByCat: Record<Cat, Field[]> = {
    motors: motorsFields,
    classifieds: dynamicClassifiedsFields,
  };

  const cats: { id: Cat; icon: typeof Car; sub: string[] }[] = [
    { id: "motors", icon: Car, sub: ["Cars", "Motorcycles", "Boats", "Heavy Vehicles", "Auto Parts"] },
    { id: "classifieds", icon: Tag, sub: Object.keys(catalog.classifiedsTree) },
  ];
    ...
    if (f.type === "make") {
      return (
        ...
        >
          ...
          {Object.keys(catalog.makeModels).sort().map((m) => <option key={m} value={m}>{m}</option>)}
        </select>
      );
    }
    if (f.type === "model") {
      const models = meta.make ? catalog.makeModels[meta.make] || [] : [];
      return (
        <select
          value={meta.model || ""}
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 md:p-8">
          {step === 1 && (
            <>
              ...
              {cat === "classifieds" && sub && catalog.classifiedsTree[sub] && (
                <div>
                  <p className="mb-2">Type</p>
                  <div className="flex flex-wrap gap-2 mb-6">
                    {catalog.classifiedsTree[sub].map((s) => (
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <div className="relative mt-1">
                      ...
                      >
                        {catalog.locations.map((loc) => <option key={loc}>{loc}</option>)}
                      </select>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                {photos.map((p, i) => (
                  <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 group">
                    ...
                    {i === 0 && (
                      <span className="absolute top-1 start-1 px-1.5 py-0.5 rounded text-[10px] font-semibold bg-blue-600 text-white">Cover</span>
                    )}
                    <button
                      onClick={() => removePhoto(i)}
                      className="absolute bottom-1.5 end-1.5 size-7 rounded-lg bg-white/90 dark:bg-slate-800/90 text-red-500 flex items-center justify-center shadow opacity-0 group-hover:opacity-100 transition hover:bg-red-50"
